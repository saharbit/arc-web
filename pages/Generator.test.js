...
test("default initials input should be 'John Doe'", async ({
  ...
  await page.goto(baseURL);
  const initialsInputValue = await page.inputValue('[placeholder="Text here"]');
  expect(initialsInputValue).toBe("John Doe");
});

test("default avatar size should be '512px'", async ({ page, baseURL }) => {
  await page.goto(baseURL);
